...
async function bootstrapProjectsPrereqs(baseUrl, apiKey, ingressId, datasetTag) {
  const normalizedBaseUrl = baseUrl.replace(/\/+$/, "");
  const skuCode = `SKU-${datasetTag}-FG`;
  const organizationName = `UEA Demo Company ${datasetTag}`;
  ...
  const createSkuResponse = await fetch(`${normalizedBaseUrl}/api/v1/inv/skus`, {
    ...
    body: JSON.stringify({
      skuCode,
      description: "UEA starter finished good for project demo",
      category: "Finished",
      uom: "EA",
  ...
  if (!createSkuResponse.ok) {
    ...
    throw new Error(`Failed to create UEA starter SKU (status ${createSkuResponse.status}): ${bodyText}`);
  }

  const skuBody = await createSkuResponse.json();
  const invSkuId = skuBody?.sku_id;
  if (!invSkuId) {
    throw new Error("UEA starter SKU response did not include sku_id.");
  }
  ...
  if (!createOrgResponse.ok) {
    ...
    throw new Error(`Failed to create UEA starter organization (status ${createOrgResponse.status}): ${bodyText}`);
  }

  const orgBody = await createOrgResponse.json();
  const invOrganizationId = orgBody?.organization_id;
  if (!invOrganizationId) {
    throw new Error("UEA starter organization response did not include organization_id.");
  }
...
const datasetTag = (process.env.STARTER_DATASET_TAG ?? "UEA-DEMO").replace(/[^A-Za-z0-9-]/g, "").toUpperCase();

assertFoundationErpEndpoint(envOverrides.baseUrl, envOverrides)
  .then(async () => {
    const prereqs = await bootstrapProjectsPrereqs(
    ...
    const args = [
      ...
      "reports/newman/uea-starter.results.json",
      "--reporter-junit-export",
      "reports/newman/uea-starter.results.xml",
      "--disable-unicode",
      "--bail"
    ];
    ...
    const focusedEnv = {
      ...
      invSkuId: prereqs.invSkuId,
      invOrganizationId: prereqs.invOrganizationId,
      invSkuCode: prereqs.skuCode,
  ...
  .catch((error) => {
    ...
    console.error(`UEA starter Postman run failed: ${error.message}`);
    process.exit(1);
  });