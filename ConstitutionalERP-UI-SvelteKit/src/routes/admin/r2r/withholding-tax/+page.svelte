...
<section class="glass-panel p-6">
	...
	{:else}
		...
		{#if groupedByCode.length > 0}
			<h3 class="mt-5 text-sm font-semibold uppercase tracking-widest text-white/60">Breakdown by WHT Code</h3>
		...
		<h3 class="mt-5 text-sm font-semibold uppercase tracking-widest text-white/60">WHT Lines ({filtered.length})</h3>
					...
					<tbody>
						{#each filtered as line (line.tax_transaction_line_id)}
							<tr class="border-b border-white/10">
								...
								<td class="px-3 py-3 font-mono text-xs text-white/50">{line.accounting_journal_id ?? '—'}</td>
