...
<section class="glass-panel p-6">
	...
	<div class="mt-4 grid gap-2 md:grid-cols-2">
		<select class="rounded-md border border-white/25 bg-[#112946] px-3 py-2 text-sm" bind:value={applicabilityFilter}>
					...
					<tbody>
						{#each filtered as line (line.tax_transaction_line_id)}
							<tr class="border-b border-white/10">
								...
								<td class="px-3 py-3">
									<span class="rounded-full px-2 py-0.5 text-xs font-semibold {line.tax_applicability === 'taxable' ? 'bg-emerald-500/20 text-emerald-300' : line.tax_applicability === 'reverse-charge' ? 'bg-blue-500/20 text-blue-300' : line.tax_applicability === 'withholding' ? 'bg-amber-500/20 text-amber-300' : 'bg-white/10 text-white/70'}">
