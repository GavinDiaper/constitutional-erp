<script lang="ts">
	...
	import { createFieldMapping, updateFieldMapping, type FieldMappingRow, type MappingStatus } from '$lib/api/mappings';
	...
	const mappingStatuses: MappingStatus[] = ['MAPPED', 'PARTIAL', 'NOT_APPLICABLE', 'GAP'];
	...
	let editorError = '';
	let editorNotice = '';
	let canonicalFields: CanonicalFieldRow[] = [];
	let mappingRows: FieldMappingRow[] = [];
	...
	let formMappingStatus: MappingStatus = 'MAPPED';
	...
	function resetEditorForm(): void {
		...
		formErpTable = '';
		formErpField = '';
		formErpFullReference = '';
		formMappingStatus = 'MAPPED';
		formTransformationNotes = '';
		formIsBidirectional = true;
	}

	function selectMapping(row: FieldMappingRow): void {
		selectedMappingId = row.id;
		formFieldId = row.field_id;
		formSystemId = row.system_id;
		formErpModule = row.erp_module ?? '';
		formErpTable = row.erp_table ?? '';
		formErpField = row.erp_field ?? '';
		formErpFullReference = row.erp_full_reference ?? '';
		formMappingStatus = row.mapping_status;
		formTransformationNotes = row.transformation_notes ?? '';
		formIsBidirectional = row.is_bidirectional === 1;
	}
	...
	async function loadEditorData(): Promise<void> {
		...
		editorError = '';
		try {
			const [fields, mappings, systems] = await Promise.all([
				queryTable<CanonicalFieldRow>('erp_canonical_field', $actorStore, 500, 0),
				queryTable<FieldMappingRow>('erp_field_mapping', $actorStore, 500, 0),
				queryTable<ErpSystem>('erp_system', $actorStore, 500, 0)
			]);

			canonicalFields = fields.data ?? [];
			mappingRows = mappings.data ?? [];
			systemsRows = systems.data ?? systemsRows;
		} catch (e) {
			editorError = e instanceof Error ? e.message : 'Failed to load mapping editor data.';
		} finally {
			editorLoading = false;
		}
	}
	...
	async function saveMapping(): Promise<void> {
		editorError = '';
		editorNotice = '';
		if (!formFieldId || !formSystemId) {
			editorError = 'Canonical field and system are required.';
			return;
		}
		...
		try {
			if (selectedMappingId) {
				await updateFieldMapping(
					selectedMappingId,
					{
						erpModule: formErpModule,
						erpTable: formErpTable,
						erpField: formErpField,
						erpFullReference: formErpFullReference,
						mappingStatus: formMappingStatus,
						transformationNotes: formTransformationNotes,
						isBidirectional: formIsBidirectional
					},
					$actorStore
				);
				editorNotice = `Updated mapping ${selectedMappingId}.`;
			} else {
				...
				editorNotice = 'Created new mapping row.';
			}
			...
			await Promise.all([loadEditorData(), loadTab('Compare Fields'), loadTab('Gap Report')]);
			if (!selectedMappingId) {
				resetEditorForm();
			}
		} catch (e) {
			editorError = e instanceof Error ? e.message : 'Failed to save mapping.';
		} finally {
			editorLoading = false;
		}
	}
	...
	onMount(() => {
		const unsub = actorStore.subscribe(() => {
			void loadTab(activeTab);
			void loadEditorData();
		});
		return unsub;
	...
	$: compareEntities = [
		...new Set(compareRows.filter((r) => !compareDomain || r.domain === compareDomain).map((r) => r.entity_name))
	].sort();
	...
	$: processPivoted = (() => {
		...
		return [...map.values()].sort(
			(a, b) => a.domain.localeCompare(b.domain) || a.sequence_order - b.sequence_order
		);
	})();
...
<section class="space-y-4">
	<div>
		<h2 class="text-2xl font-semibold">ERP Mapping Tool</h2>
		<p class="muted mt-2 text-sm">Standalone explorer and editor for ERP mapping V2 tables.</p>
	</div>

	<Tabs tabs={[...TABS]} selected={activeTab} onSelect={switchTab} />

	<div class="flex flex-wrap items-center gap-2 rounded-md border border-slate-300 dark:border-white/20 p-3">
		<label class="text-xs uppercase tracking-[0.12em] text-slate-500 dark:text-white/60" for="actor-select">Actor</label>
		<select
			id="actor-select"
			class="rounded-md border dark:border-white/25 border-slate-300 bg-(--input-bg) px-3 py-2 text-xs"
			bind:value={selectedActorId}
			on:change={() => setActorById(selectedActorId)}
		>
		...
		<button
			type="button"
			class="rounded-md border dark:border-white/35 border-slate-300 px-3 py-2 text-xs dark:hover:bg-white/10 hover:bg-slate-500/10"
			on:click={() => {
				void loadEditorData();
					...
					<tbody>
						{#each filteredProcess as row (row.process_id)}
							<tr class="border-b dark:border-white/10 border-slate-200 align-top hover:dark:bg-white/5 hover:bg-slate-50">
								...
								<td class="px-3 py-2 font-medium max-w-45">{row.process_name}</td>
								<td class="px-3 py-2 font-mono dark:text-white/50 text-slate-400">{row.canonical_command ?? '—'}</td>
								{#each processVisibleSystems as sid (sid)}
									{@const sys = row.systems[sid]}
									<td class="px-3 py-2 border-l dark:border-white/10 border-slate-200 {statusCellClass(sys?.status ?? null)} max-w-50">
	...
	<section class="space-y-3 rounded-md border border-slate-300 dark:border-white/20 p-4">
		<div class="flex items-center justify-between gap-2">
			<h3 class="text-sm font-semibold uppercase tracking-[0.12em] text-slate-500 dark:text-white/60">Manage Field Mappings</h3>
			<button
				type="button"
				class="rounded-md border dark:border-white/35 border-slate-300 px-3 py-2 text-xs dark:hover:bg-white/10 hover:bg-slate-500/10"
				on:click={() => {
			...
			>
				{editorLoading ? 'Loading…' : 'Refresh Editor'}
			</button>
		</div>

		{#if editorError}
			<p class="rounded-md border border-red-500/55 bg-red-500/10 p-3 text-sm text-red-400">{editorError}</p>
		{/if}
		{#if editorNotice}
			<p class="rounded-md border border-emerald-500/55 bg-emerald-500/10 p-3 text-sm text-emerald-300">{editorNotice}</p>
		{/if}

		<div class="grid gap-2 md:grid-cols-2">
			<label class="flex flex-col gap-1 text-xs">
				<span class="muted">Canonical field</span>
				<select class="rounded-md border dark:border-white/25 border-slate-300 bg-(--input-bg) px-3 py-2" bind:value={formFieldId} disabled={Boolean(selectedMappingId)}>
					<option value="">Select field</option>
					{#each canonicalFields as field (field.field_id)}
						<option value={field.field_id}>{field.domain} - {field.canonical_field} ({field.field_id})</option>
					{/each}
				</select>
			</label>
			...
			<label class="flex flex-col gap-1 text-xs">
				...
				<select class="rounded-md border dark:border-white/25 border-slate-300 bg-(--input-bg) px-3 py-2" bind:value={formSystemId} disabled={Boolean(selectedMappingId)}>
					<option value="">Select system</option>
					{#each systemsRows as system (system.system_id)}
						<option value={system.system_id}>{system.system_id} - {system.name}</option>
					{/each}
				</select>
			</label>

			<label class="flex flex-col gap-1 text-xs">
				<span class="muted">Mapping status</span>
				<select class="rounded-md border dark:border-white/25 border-slate-300 bg-(--input-bg) px-3 py-2" bind:value={formMappingStatus}>
			...
			<label class="flex flex-col gap-1 text-xs">
				<span class="muted">ERP module</span>
				<input class="rounded-md border dark:border-white/25 border-slate-300 bg-(--input-bg) px-3 py-2" bind:value={formErpModule} placeholder="Order Management" />
			</label>

			<label class="flex flex-col gap-1 text-xs">
				<span class="muted">ERP table</span>
				<input class="rounded-md border dark:border-white/25 border-slate-300 bg-(--input-bg) px-3 py-2" bind:value={formErpTable} placeholder="DOO_HEADERS_ALL" />
			</label>

			<label class="flex flex-col gap-1 text-xs">
				<span class="muted">ERP field</span>
				<input class="rounded-md border dark:border-white/25 border-slate-300 bg-(--input-bg) px-3 py-2" bind:value={formErpField} placeholder="HEADER_ID" />
			</label>
		</div>

		<label class="flex flex-col gap-1 text-xs">
			<span class="muted">ERP full reference</span>
			<input class="rounded-md border dark:border-white/25 border-slate-300 bg-(--input-bg) px-3 py-2" bind:value={formErpFullReference} placeholder="DOO_HEADERS_ALL.HEADER_ID" />
		</label>

		<label class="flex flex-col gap-1 text-xs">
			<span class="muted">Transformation notes</span>
			<textarea class="rounded-md border dark:border-white/25 border-slate-300 bg-(--input-bg) px-3 py-2" rows="2" bind:value={formTransformationNotes} placeholder="Any conversion or truncation rules"></textarea>
		</label>

		<label class="flex items-center gap-2 text-xs">
			<input type="checkbox" bind:checked={formIsBidirectional} />
			<span class="muted">Bidirectional mapping</span>
		</label>

		<div class="flex flex-wrap gap-2">
			<button type="button" class="rounded-md border border-emerald-500/55 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-300" on:click={saveMapping} disabled={editorLoading}>
				{selectedMappingId ? 'Update mapping' : 'Create mapping'}
			</button>
			<button type="button" class="rounded-md border dark:border-white/35 border-slate-300 px-3 py-2 text-xs dark:hover:bg-white/10 hover:bg-slate-500/10" on:click={resetEditorForm}>
				Reset form
			</button>
		</div>
		...
		<div class="overflow-x-auto rounded-md border dark:border-white/15 border-slate-300">
			<table class="min-w-full text-left text-xs">
				...
				<tbody>
					{#each mappingRowsSorted as row (row.id)}
						<tr class="border-b dark:border-white/10 border-slate-200 align-top">
							<td class="px-3 py-2 font-mono">{row.id}</td>
							<td class="px-3 py-2 font-mono">{row.field_id}</td>
							<td class="px-3 py-2">{row.system_id}</td>
							<td class="px-3 py-2">{row.mapping_status}</td>
							<td class="px-3 py-2">{row.erp_full_reference ?? '—'}</td>
							<td class="px-3 py-2">
								<button type="button" class="rounded border dark:border-white/35 border-slate-300 px-2 py-1 text-[11px]" on:click={() => selectMapping(row)}>
									Edit
								</button>
							</td>
						</tr>
