...
export function queryTable<T>(
	...
	return fetchHubJson<QueryTableResponse<T>>(`/api/v1/query/${table}?${params.toString()}`, actor);
}

export function queryRow<T>(table: string, id: string, actor: ActorContext): Promise<QueryRowResponse<T>> {
	return fetchHubJson<QueryRowResponse<T>>(`/api/v1/query/${table}/${encodeURIComponent(id)}`, actor);
}
